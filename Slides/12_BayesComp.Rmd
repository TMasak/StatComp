...
Let $U^{(0)} = \epsilon_0$ and $U^{(t+1)} = X^{(t)} + \epsilon_t$, $t \geq 1$, where $\epsilon_0,\epsilon_1,\ldots$ drawn independently from a symmetric density, leading to $k(x,y) = g(y-x)$.

Verifying detailed balance is easy in this case:

* detailed balance: $k(x,u) f(x) = k(u,x) f(u)$ for $x \stackrel{?}{\leadsto} u$
* $k(x,u)$ is given implicitly as the mixture of
  ...
  - staying at $x$ with probability $1-\alpha(x,u)$
    - this is symmetric w.r.t. time flow, so we do not have to worry about it
