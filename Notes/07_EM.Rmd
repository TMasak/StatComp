...
$$
\begin{split}
H\big(\theta,\theta^{(l-1)}\big) &= \E_{\theta^{(l-1)}} \left[ \log\frac{f_\theta(X_{miss}|X_{obs})}{f_{\theta^{(l-1)}}(X_{miss}|X_{obs})} \middle| X_{obs} \right] + H\big(\theta^{(l)},\theta^{(l-1)}\big) \\
&\leq \log \underbrace{\E_{\theta^{(l-1)}} \left[ \frac{f_\theta(X_{miss}|X_{obs})}{f_{\theta^{(l-1)}}(X_{miss}|X_{obs})} \middle| X_{obs} \right]}_{= \int \frac{f_\theta(x_{miss}|X_{obs})}{f_{\theta^{(l-1)}}(x_{miss}|X_{obs})} f_{\theta^{(l-1)}}(x_{miss}|X_{obs}) d x_{miss} = 1} + H\big(\theta^{(l)},\theta^{(l-1)}\big)
\end{split}
$$
and so indeed $H\big(\theta,\theta^{(l-1)}\big) \leq H\big(\theta^{(l)},\theta^{(l-1)}\big)$. <div style="text-align: right">**Q.E.D.**</div>
