...
The most natural estimator for $F$ is the empirical distribution function $\widehat{F}_n$, i.e. $\widehat{F}_n(x) = \frac{1}{n} \sum_{i=1}^n \mathbb{I}_{[X_i \leq x]}$. If we fix the *bandwidth* $h = h_n$ (at something "small" and depending on $n$, we need $h_n \to 0_+$ for $n \to 0$ in order to have any hope for consistency) and plug in the EDF to the previous equation, we obtain a density estimator
...
Also, even when we calculate the naive estimator $\widehat{f}(x)$ on a dense grid of points (compared to a histogram) we will have exactly the histogram's value in the middle of every histogram bin. Hence the naive estimator $\widehat{f}(x)$ can be essentially though of as a "moving histogram"(so *origin* does not matter anymore).
...
Let us calculate the bias and the variance (note they are both pointwise, i.e. depending on $x$, just as the MSE is). For the bias:
$$
\E \widehat{f}(x) = \E \frac{1}{n h} \sum_{i=1}^n K\left(\frac{X_i - x}{h} \right) = \frac{1}{h} \int K\left(\frac{y - x}{h} \right) f(y) d y
$$
...
$$\mathrm{var}\big( \widehat{f}(x) \big) = \frac{1}{nh^2} \mathrm{var}\left(\star\right) = \frac{1}{nh^2} \E \star^2 - \frac{1}{nh^2} \big(\E \star \big)^2 = \frac{1}{nh^2} \int \left( K\left(\frac{y - x}{h} \right) \right)^2 f(y) dy - \frac{1}{nh^2} \left(\int K\left(\frac{y - x}{h} \right)  f(y) dy \right)^2 $$
and hence for $h=h_n \to 0$ and $n h_n \to \infty$ we have (using the same substitution and Taylor as above)
$$
\begin{split}
\mathrm{var}\big( \widehat{f}(x) \big) &= \frac{1}{nh} \underbrace{\frac{1}{h} \int \left( K\left(\frac{y - x}{h} \right) \right)^2 f(y) dy}_{\int f(x+hz) [K(z)]^2 dz} - \underbrace{\frac{1}{n}\big[ f(x) + bias(x) \big]^2}_{\mathcal{O}(n^{-1}) = \mathcal{o}(n^{-1} h^{-1})} \\
&= \frac{1}{nh} f(x) \int \big[K(z)\big]^2 dz + \mathcal{o}\left( \frac{1}{nh} \right)
\end{split}
$$

The calculations above lead to the asymptotically optimal choice of bandwidth (ignore the o-terms, derive the MSE by $h$ and set it to zero):
...
* It reveals the $h_n \propto n^{-1/5}$ is the asymptotically optimal rate for the bandwidth. Plugging the optimal bandwidth back to the formulas for bias and variance, both the squared bias and the variance will be of order $\mathcal{O}(n^{-4/5})$, which is indeed the optimal rate of convergence. Notice that this rate is slower than the usual $1/n$ we achieve with parametric methods (in the correct models!), which is the price we pay for the flexibility offered by the non-parametric approach.
...
`ggplot2`'s `geom_smooth()` can use any smoother provided to it as a function, but by default it uses `loess()` (from the base R distribution) or `mgcv`'s `gam()` (using spline smoothing, which we will not cover in this course)

program your own version of local linear regression smoother using the tricube kernel ($\propto 1 - |x|^3$) and compare it's behavior to `loess()` (which should be exactly the local linear regression with a tricube kernel, but who knows. Apparently `loess` returns a spline smoother of what would be computed using LOESS of Cleveland, which is equivalent to NW estimator)

* Simulation study: On the slide "Bandwidth > Kernel" a claim is made, but only a visual evidence is given. Check out for yourself whether the claim is true.
  - Compare different kernels for KDE, draw data from a Gaussian mixture and try KDE for Epanechnikov, Gaussian, Tricube and Rectangular kernels. Which one leads to the smallest MISE?
  - Display your results for sample sizes $n=50,100,200,500$. The bandwidth $h$ is a nuissance parameter here: calculate everything on a grid of values for $h$ and always choose the minimum attained error.

Simulation study: how much better is a 2-step local bw selection compared to a reference method?

Simulation study: probe the $n^{-4/5}$ rate for kernel density estimators

Programm a KDE smoother using FFT yourself.

KDE and Kernel Regression are interesting for projects in general. We only touch the surface, there is much more to be explored, and potential non-trivial simulation studies are everywhere, it also really fits the spirit of this course. The way to make such a project a top-project would be to make a survey like the Wickham's one about local linear estimators.

Omelka E12 ma peknou simulacni studii porovnavajici ruzne bandwidth selection strategies.
