...
**Example** Example 57 from Omelka's course notes (standardized bi-variate distribution) above and Example 60 for Cauchy distribution with known scale = 1 here.
...
A turning point came from the great [John von Neumann](https://en.wikipedia.org/wiki/John_von_Neumann), father of
...
John von Neumann advocated that sampling should *not be* random, they should only *appear to be* random by passing certain criteria (pseudo-randomness). In particular, a generated sequence should satisfy
...
where $a,c,m$ and $X_0$ are cleverly chosen fixed numbers, can fulfill the requirements above. The reason why "short enough" is written above the requirements in the parentheses is that the linear congruential method will naturally produce only a finite sequence of numbers before starting to repeat itself. A length of such sequence can however be very large (e.g. $10^{12}$) depending on the choice of the constants, so this is hardly a practical problem.
...
Now that we know how to generate data from the $\mathcal{U}[0,1]$ distribution (at least for most practical purposes), we can formally use the following theorem to generate data from any other distribution.
...
Assume we wish to simulate from a *target* density $f$ and we know how to simulate from a *proposal* density $g$. Assume $f$ and $g$ have the same support (i.e. $\forall x: \,f(x) > 0 \Leftrightarrow g(x) > 0$) and there exists $c \geq 1$ such that $\forall x : \,c f(x) \leq g(x)$. The following algorithm can then be used to draw a sample $X$ from $f$:
...
In the example of the standard uniform proposal and the beta $\mathcal{B}(2,5)$ target below, it is easy to see that the algorithm works (really generates from the target $f$) and that the probability of stopping in step 3 (the acceptance probability) of the algorithm above is equal to $1/M$.
...
I = \E\big[ f(U) \big]
\]
is approximated stochastically by $\bar{f}_N = N^{-1} \sum_{n=1}^N f(X_n)$. Due to the law of large numbers, $S_N \to I$ almost surely. Compared to the deterministic approaches, Monte Carlo has the advantage of easy convergence monitoring: $\bar{f}_N$ is a random variable arising as a mean of i.i.d. random variables. It is unbiased and its variance can be naturally estimated as $v_n$ below. Hence by the central limit theorem we have
\[
\sqrt{N}\frac{\bar{f}_N - I}{v_N} \stackrel{\cdot}{\sim} \mathcal{N}(0,1) \,, \quad \text{where} \quad v_N = \frac{1}{N} \sum_{n=1}^N \big[ f(X_n) - \bar{f}_N \big]^2
\]
...
\[
\mathrm{avar}\big(\widehat{I}\big) = \frac{1}{N} \int_\mathcal{X} \left( \frac{f(x)}{g(x)} -  \E_g \left[ \frac{f(X)}{g(X)} \right] \right) g(x) d x.
\]
In particular, the variance is zero if $f(x)/g(x) = 1$ on the domain $\mathcal{X}$, and is small if $f(x)/g(x) \approx 1$ on $\mathcal{X}$.
...
plot(cumsum(wein*(x>2)*(x<6))/cumsum(rep(1,10^5)),type="l",ylim=c(0,0.05))
abline(a=pnorm(6)-pnorm(2),b=0,col="sienna")

x <- 6*(1:1000)/1001
plot(x,dnorm(x)/dexp(x),type="l")
points(x,dexp(x),type="l",col="blue")
abline(v=c(2,6),lty=2)
```

**Example:** *(Exponential reference, Cauchy target)*
...
# boxplot(wein/sum(wein))
plot(cumsum(wein*(x>2)*(x<6))/cumsum(rep(1,10^5)),type="l",ylim=c(0,0.3))
abline(a=pcauchy(6)-pcauchy(2),b=0,col="sienna")

x <- 6*(1:1000)/1001
plot(x,dcauchy(x)/dexp(x),type="l",ylim=c(0,3))
points(x,dexp(x),type="l",col="blue")
abline(v=c(2,6),lty=2)
```

**Example:** *(Gaussian reference, Cauchy target)*
...
# boxplot(wein/sum(wein))
plot(cumsum(wein*(x>2)*(x<6))/cumsum(rep(1,10^3)),type="l",ylim=c(0,0.3))
abline(a=pcauchy(6)-pcauchy(2),b=0,col="sienna")

x <- 6*(1:1000)/1001
plot(x,dcauchy(x)/dnorm(x),type="l",ylim=c(0,10))
